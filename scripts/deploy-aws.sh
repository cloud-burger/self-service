...
argocd app create self-service --repo https://github.com/cloud-burger/self-service.git --revision main --path iac/gitops/argocd/self-service --dest-server https://kubernetes.default.svc --dest-namespace argocd --directory-recurse --sync-policy automated --sync-option ApplyOutOfSyncOnly=true
